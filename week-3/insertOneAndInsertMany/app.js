...
MongoClient.connect('mongodb://localhost:27017/social', function(err, db) {
    ...
    twitterClient.stream('statuses/filter', {track: "marvel"}, function(stream) {
        stream.on('data', function(status) { // first of two event handlers, this one to catch data
        ...
        stream.on('error', function(error) { // this event handler to catch any errors
            throw error;
        });
    });
